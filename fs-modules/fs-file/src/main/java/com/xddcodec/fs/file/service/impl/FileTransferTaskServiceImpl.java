...
public class FileTransferTaskServiceImpl extends ServiceImpl<FileTransferTaskMapper, FileTransferTask> implements FileTransferTaskService {
    ...
    private final FileAuditLogService auditLogService;
...
//        }
    ...
    public String initUpload(InitUploadCmd cmd) {
        ...
        try {
            ...
            task.setReplacedFileId(cmd.getReplacedFileId());
            this.save(task);
            ...
            log.info("初始化上传成功: fileName={}, replacedFileId={}", cmd.getFileName(), cmd.getReplacedFileId());
    ...
    public CheckUploadResultVO checkUpload(CheckUploadCmd cmd) {
        ...
        try {
            ...
            nameQuery.where(FILE_INFO.WORKSPACE_ID.eq(workspaceId))
                    .and(FILE_INFO.PARENT_ID.eq(task.getParentId()))
                    .and(FILE_INFO.DISPLAY_NAME.eq(cmd.getFileName()))
                    .and(FILE_INFO.IS_DELETED.eq(false))
                    .and(FILE_INFO.IS_DIR.eq(false));
            FileInfo duplicateByName = fileInfoService.getOne(nameQuery);

            if (duplicateByName != null && !"replace".equals(cmd.getAction()) && !"keep".equals(cmd.getAction())) {
                log.info("检测到同名文件，需要用户确认: taskId={}, duplicateFileId={}", taskId, duplicateByName.getId());
                updateTaskStatus(task, TransferTaskStatus.initialized);
                return CheckUploadResultVO.builder()
                        .isQuickUpload(false)
                        .taskId(taskId)
                        .hasDuplicate(true)
                        .duplicateFile(CheckUploadResultVO.DuplicateFileInfo.builder()
                                .fileId(duplicateByName.getId())
                                .fileName(duplicateByName.getDisplayName())
                                .fileSize(duplicateByName.getSize())
                                .uploadTime(duplicateByName.getUploadTime() != null ? duplicateByName.getUploadTime().toString() : null)
                                .build())
                        .message(I18nUtils.getMessage("file.duplicate.detected"))
                        .build();
            }

            if ("replace".equals(cmd.getAction()) && duplicateByName != null) {
                log.info("用户选择替换同名文件: taskId={}, duplicateFileId={}", taskId, duplicateByName.getId());
                return handleReplaceFile(task, duplicateByName, cmd.getFileMd5(), storagePlatformSettingId);
            }

            if ("keep".equals(cmd.getAction()) && duplicateByName != null) {
                String uniqueName = fileInfoService.generateUniqueName(
                        workspaceId,
                        task.getParentId(),
                        cmd.getFileName(),
                        false,
                        duplicateByName.getId(),
                        storagePlatformSettingId
                );
                task.setFileName(uniqueName);
                log.info("保留两者，生成唯一文件名: taskId={}, uniqueName={}", taskId, uniqueName);
            }
    ...
    private CheckUploadResultVO handleReplaceFile(FileTransferTask task, FileInfo existingFile,
                                                  String fileMd5, String storagePlatformSettingId) {
        String taskId = task.getTaskId();
        try {
            log.info("开始替换文件: taskId={}, existingFileId={}", taskId, existingFile.getId());
            ...
            task.setUploadId(uploadId);
            task.setReplacedFileId(existingFile.getId());

            updateTaskStatus(task, TransferTaskStatus.uploading);

            transferSseService.sendStatusEvent(task.getUserId(), taskId,
                    TransferTaskStatus.uploading.name(), I18nUtils.getMessage("task.replace.started"));
            ...
            return CheckUploadResultVO.builder()
                    ...
                    .duplicateFile(CheckUploadResultVO.DuplicateFileInfo.builder()
                            ...
                            .fileName(existingFile.getDisplayName())
                            .fileSize(existingFile.getSize())
                            .uploadTime(existingFile.getUploadTime() != null ? existingFile.getUploadTime().toString() : null)
                            .build())
                    .message(I18nUtils.getMessage("task.replace.started"))
                    .build();
        } catch (Exception e) {
            log.error("替换文件处理失败: taskId={}", taskId, e);
            throw new StorageOperationException(I18nUtils.getMessage("task.replace.failed", new Object[]{e.getMessage()}), e);
        }
    }
    ...
    protected CheckUploadResultVO handleQuickUpload(FileTransferTask task,
        ...
        try {
            ...
            String displayName = task.getFileName();
            ...
            String replacedFileId = task.getReplacedFileId();
            if (replacedFileId != null && !replacedFileId.isEmpty()) {
                FileInfo oldFile = fileInfoService.getById(replacedFileId);
                if (oldFile != null) {
                    try {
                        fileVersionService.createVersion(
                                fileId,
                                oldFile.getObjectKey(),
                                oldFile.getOriginalName(),
                                oldFile.getSize(),
                                oldFile.getContentMd5(),
                                "文件被替换前自动保存的版本",
                                oldFile.getUserId()
                        );
                        log.info("秒传-旧文件版本已保存: replacedFileId={}", replacedFileId);
                    } catch (Exception e) {
                        log.warn("秒传-保存旧文件版本失败: replacedFileId={}", replacedFileId, e);
                    }
                    oldFile.setIsDeleted(true);
                    oldFile.setDeletedTime(LocalDateTime.now());
                    fileInfoService.updateById(oldFile);
                }
            }
            
            // 记录日志
            auditLogService.logFileUpload(fileId, displayName, task.getUserId());
    ...
    public FileInfo doMergeChunks(String taskId) {
        ...
        try {
            ...
            String replacedFileId = task.getReplacedFileId();
            if (replacedFileId != null && !replacedFileId.isEmpty()) {
                log.info("检测到文件替换，保存旧版本: replacedFileId={}, newFileId={}", replacedFileId, fileId);
                FileInfo oldFile = fileInfoService.getById(replacedFileId);
                if (oldFile != null) {
                    try {
                        fileVersionService.createVersion(
                        ...
                        log.info("旧文件版本已保存: replacedFileId={}", replacedFileId);
                    } catch (Exception e) {
                        log.warn("保存旧文件版本失败，继续替换流程: replacedFileId={}", replacedFileId, e);
                    }
                    auditLogService.logFileDelete(replacedFileId, oldFile.getDisplayName(), task.getUserId(), "replace");
                    oldFile.setIsDeleted(true);
                    oldFile.setDeletedTime(LocalDateTime.now());
                    fileInfoService.updateById(oldFile);
                    log.info("旧文件已标记为删除: replacedFileId={}", replacedFileId);
                }
            }
            
            // 记录日志
            auditLogService.logFileUpload(fileId, task.getFileName(), task.getUserId());
