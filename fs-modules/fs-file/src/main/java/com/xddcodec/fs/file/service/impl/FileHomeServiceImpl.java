...
import com.xddcodec.fs.file.domain.vo.FileTypeStatsVO;
...
public class FileHomeServiceImpl implements FileHomeService {
    ...
    public FileHomeVO getFileHomes(FileHomeUsedBytesQry qry) {
        ...
        List<FileTypeStatsVO> fileTypeStats = getFileTypeStats();
        fileHomeVO.setFileTypeStats(fileTypeStats);
    ...
    private List<FileTypeStatsVO> getFileTypeStats() {
        String workspaceId = WorkspaceContext.getWorkspaceId();
        String storageId = StoragePlatformContextHolder.getConfigId();

        QueryWrapper queryWrapper = QueryWrapper.create()
                .select(FILE_INFO.SUFFIX, FILE_INFO.SIZE)
                .where(FILE_INFO.WORKSPACE_ID.eq(workspaceId))
                .and(FILE_INFO.STORAGE_PLATFORM_SETTING_ID.eq(storageId))
                .and(FILE_INFO.IS_DIR.eq(false))
                .and(FILE_INFO.IS_DELETED.eq(false));
        ...
        Map<FileTypeEnum.FileCategory, Long> countMap = new EnumMap<>(FileTypeEnum.FileCategory.class);
        Map<FileTypeEnum.FileCategory, Long> sizeMap = new EnumMap<>(FileTypeEnum.FileCategory.class);

        for (FileInfo f : files) {
            FileTypeEnum fileType = FileTypeEnum.fromSuffix(f.getSuffix());
            FileTypeEnum.FileCategory category = fileType.getCategory();
            countMap.merge(category, 1L, Long::sum);
            sizeMap.merge(category, f.getSize() != null ? f.getSize() : 0L, Long::sum);
        }

        List<FileTypeStatsVO> result = new ArrayList<>();
        for (FileTypeEnum.FileCategory cat : FileTypeEnum.FileCategory.values()) {
            Long count = countMap.getOrDefault(cat, 0L);
            Long size = sizeMap.getOrDefault(cat, 0L);
            result.add(new FileTypeStatsVO(cat.getCode(), cat.getName(), count, size));
        }
